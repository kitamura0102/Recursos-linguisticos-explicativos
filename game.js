...
/* ============================================================
   FEEDBACK
============================================================ */
function showFeedback(col, expected) {
  const fb = document.getElementById('feedback');
  const colName = col === 'causa' ? 'causa' : 'reformulación';
  const hint = expected === NO_CONNECTOR
    ? `En esta fila no hay conector de ${colName}. Usa "No tiene conector".`
    : `El conector de ${colName} aquí es: "${expected}".`;
  fb.textContent   = '✗  ' + hint;
